...
let currentCarouselItem = ref < IMenuItem | undefined > (props.carouselItems ?.[0]);
...
const showSelectedMenu = (event: MouseEvent) => {
    ...
    if (target) {
        ...
        currentCarouselItem.value = props.carouselItems && props.carouselItems[index]
...
<section>
    <ul class="flex justify-between">
        <li v-for="(item, index) in props.carouselItems" :key="index" @click="showSelectedMenu" class="menuItem">
            <img :src="item.src" alt="" class="absolute h-24 w-24 rounded-full object-cover
